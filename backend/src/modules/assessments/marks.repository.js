...
// ---------- LIST MARKS ----------
exports.list = (schoolId, { assessment_id, task_id, grade_id, stream_id, subject_id, student_id } = {}) => {
  const params = [schoolId];
  let where = "a.school_id=?";
  if (assessment_id) { where += " AND m.assessment_id=?"; params.push(assessment_id); }
  if (task_id) { where += " AND m.task_id=?"; params.push(task_id); }
  ...
  if (stream_id) { where += " AND stu.stream_id=?"; params.push(stream_id); }
...
exports.roster = async (schoolId, taskId) => {
  ...
  let where = "grade_id=? AND status='active'";
  if (t.stream_id) { where += " AND stream_id=?"; params.push(t.stream_id); }
  const students = await query(
    `SELECT id, first_name, last_name, admission_number, gender, grade_id, stream_id
       FROM students WHERE ${where} ORDER BY first_name, last_name`,
    params,
  );
  ...
  return {
    ...
    students: students.map((s) => ({ ...s, mark: byStudent.get(s.id) || null })),
  };
};

// ---------- BULK SAVE ----------
// Accepts: { assessment_id, task_id?, items:[{student_id, subject_id, score, out_of, status, remarks}] }
exports.bulkSave = async (schoolId, { assessment_id, task_id, recorded_by, items }) => {
  ...
  for (const it of items) {
    ...
    const score = it.score === "" || it.score === null || it.score === undefined
      ? null
      : Number(it.score);
    ...
    if (existing) {
      ...
      await execute(
        `INSERT INTO assessment_marks
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        [
          id, assessment_id, task_id || null, it.student_id, it.subject_id,
          score, outOf,
          al?.code || null, al?.band_code || null, al?.points ?? null,
          it.status || (score !== null ? "present" : "pending"),
          it.remarks || null, recorded_by || null,
        ],
      );
      saved.push(id);
    }
