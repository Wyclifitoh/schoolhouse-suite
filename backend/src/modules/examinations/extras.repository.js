...
const AT = {
  ...
  remove: (id, schoolId) =>
    query(`DELETE FROM assessment_types WHERE id = ? AND school_id = ?`, [
      id,
      schoolId,
    ]),
};
...
const GS = {
  list: (schoolId) =>
    query(
      `SELECT * FROM grading_scales WHERE school_id = ? ORDER BY is_default DESC, name`,
      [schoolId],
    ),
  get: async (id, schoolId) => {
    const scale = await queryOne(
      `SELECT * FROM grading_scales WHERE id = ? AND school_id = ?`,
      [id, schoolId],
    );
    if (!scale) return null;
    scale.bands = await query(
      `SELECT * FROM grading_bands WHERE scale_id = ? ORDER BY min_score DESC`,
      [id],
    );
    scale.levels = await query(
      `SELECT * FROM cbc_performance_levels WHERE scale_id = ? ORDER BY sort_order`,
      [id],
    );
    return scale;
  },
  create: async (schoolId, b) => {
    ...
    if (b.is_default) {
      await query(
        `UPDATE grading_scales SET is_default = 0 WHERE school_id = ? AND id <> ?`,
        [schoolId, id],
      );
    }
  ...
  update: async (id, schoolId, b) => {
    ...
    if (Array.isArray(b.bands)) {
      await query(`DELETE FROM grading_bands WHERE scale_id = ?`, [id]);
      for (const band of b.bands) {
        await query(
          `INSERT INTO grading_bands (id, scale_id, min_score, max_score, grade, points, descriptor, remark)
           VALUES (?,?,?,?,?,?,?,?)`,
    ...
    if (Array.isArray(b.levels)) {
      await query(`DELETE FROM cbc_performance_levels WHERE scale_id = ?`, [
        id,
      ]);
      for (const lvl of b.levels) {
        await query(
  ...
  remove: (id, schoolId) =>
    query(`DELETE FROM grading_scales WHERE id = ? AND school_id = ?`, [
      id,
      schoolId,
    ]),
};
...
const COMP = {
  list: (schoolId, { subject_id, grade_id } = {}) => {
    ...
    return query(
      `SELECT * FROM cbc_competencies WHERE ${where.join(" AND ")} ORDER BY name`,
      p,
    );
  },
  create: async (schoolId, b) => {
  ...
  update: async (id, schoolId, b) => {
    ...
    return queryOne(
      `SELECT * FROM cbc_competencies WHERE id = ? AND school_id = ?`,
      [id, schoolId],
    );
  },
  remove: (id, schoolId) =>
    query(`DELETE FROM cbc_competencies WHERE id = ? AND school_id = ?`, [
      id,
      schoolId,
    ]),
};
...
const OBS = {
  list: (schoolId, session, { student_id, competency_id } = {}) => {
    ...
    if (competency_id) {
      where.push("competency_id = ?");
      p.push(competency_id);
    }
    return query(
  ...
  remove: (id, schoolId) =>
    query(`DELETE FROM cbc_observations WHERE id = ? AND school_id = ?`, [
      id,
      schoolId,
    ]),
};
...
const RC = {
  listTemplates: (schoolId) =>
    query(
      `SELECT * FROM report_card_templates WHERE school_id = ? ORDER BY is_default DESC, name`,
      [schoolId],
    ),
  createTemplate: async (schoolId, b) => {
    ...
    if (b.is_default) {
      await query(
        `UPDATE report_card_templates SET is_default = 0 WHERE school_id = ? AND id <> ?`,
        [schoolId, id],
      );
    }
  ...
  removeTemplate: (id, schoolId) =>
    query(`DELETE FROM report_card_templates WHERE id = ? AND school_id = ?`, [
      id,
      schoolId,
    ]),
