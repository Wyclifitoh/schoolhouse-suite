...
const sessionWhere = (alias, session) => {
  const w = [], p = [];
  if (session?.academicYearId) { w.push(`${alias}.academic_year_id = ?`); p.push(session.academicYearId); }
  if (session?.termId)         { w.push(`${alias}.term_id = ?`);          p.push(session.termId); }
...
const listExams = async (schoolId, session = {}, filters = {}) => {
  ...
  if (ses.sql) { where.push(ses.sql); params.push(...ses.params); }
  if (filters.status) { where.push("e.status = ?"); params.push(filters.status); }
  if (filters.assessment_type_id) { where.push("e.assessment_type_id = ?"); params.push(filters.assessment_type_id); }
  ...
  return rows.map((r) => ({ ...r, classes: r.classes ? r.classes.split(", ").filter(Boolean) : [] }));
};

const getExam = (id, schoolId) =>
  queryOne("SELECT * FROM exams WHERE id = ? AND school_id = ?", [id, schoolId]);
...
const createExam = async (data) => {
  ...
  await execute(
    `INSERT INTO exams
     ...
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [
      id, data.school_id, data.academic_year_id || null, data.term_id || null,
      data.name, data.type || "CAT",
      data.assessment_type_id || null, data.grading_scale_id || null,
      data.weight ?? 100,
      data.term || null, data.start_date || null, data.end_date || null,
      data.status || "DRAFT", data.curriculum_type || "CBC",
    ...
    for (const s of data.subjects) {
      await execute(
        `INSERT INTO exam_subjects (id, exam_id, subject_id, subject_name, max_marks, pass_mark, weight, grading_scale_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          uuidv4(), id, s.subject_id || null, s.subject_name,
          s.max_marks ?? 100, s.pass_mark ?? null, s.weight ?? 100,
...
const updateExam = async (id, schoolId, data) => {
  const allowed = [
    "name","type","term","start_date","end_date","description","term_id","academic_year_id",
    "curriculum_type","assessment_type_id","grading_scale_id","weight",
  ];
  const entries = Object.entries(data).filter(([k, v]) => allowed.includes(k) && v !== undefined);
  if (entries.length) {
    const fields = entries.map(([k]) => `${k} = ?`).join(", ");
    await execute(
      `UPDATE exams SET ${fields} WHERE id = ? AND school_id = ?`,
...
const deleteExam = (id, schoolId) =>
  execute("DELETE FROM exams WHERE id = ? AND school_id = ?", [id, schoolId]);

// ===== Lifecycle =====
const transitionExam = async ({ id, schoolId, action, actorId, actorRole, reason }) => {
  const exam = await getExam(id, schoolId);
  if (!exam) { const e = new Error("Exam not found"); e.statusCode = 404; throw e; }
  const next = assertTransition(exam.status, action, actorRole);

  const fields = ["status = ?"];
  const params = [next];
  if (next === "APPROVED") { fields.push("published_at = NOW()"); }
  if (next === "LOCKED")   { fields.push("locked_at = NOW()", "locked_by = ?"); params.push(actorId); }
  ...
  // Recompute rankings whenever the exam is officially approved.
  if (next === "APPROVED") {
    try {
      const { recomputeRankings } = require("./extras.repository");
      await recomputeRankings(schoolId, id, { academicYearId: exam.academic_year_id, termId: exam.term_id });
    } catch (_) { /* non-fatal */ }
  }
...
const listSchedules = async (schoolId, examId) => {
  const where = examId ? "WHERE e.school_id = ? AND s.exam_id = ?" : "WHERE e.school_id = ?";
...
const createSchedule = async (data) => {
  ...
  await execute(
    `INSERT INTO exam_schedules
     ...
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [
      id, data.exam_id, data.subject_id || null, data.subject_name,
      data.grade_id || null, data.stream_id || null,
      data.exam_date || null, data.start_time || null, data.end_time || null,
      data.room || null, data.full_marks || 100, data.pass_marks || 50,
...
const updateSchedule = async (id, data) => {
  const allowed = ["subject_name","grade_id","stream_id","exam_date","start_time","end_time","room","full_marks","pass_marks","invigilator_id","subject_id"];
  const entries = Object.entries(data).filter(([k, v]) => allowed.includes(k) && v !== undefined);
  if (entries.length) {
    const fields = entries.map(([k]) => `${k} = ?`).join(", ");
    await execute(
      `UPDATE exam_schedules SET ${fields} WHERE id = ?`,
      [...entries.map(([, v]) => v), id],
    );
  }
  return queryOne("SELECT * FROM exam_schedules WHERE id = ?", [id]);
...
const deleteSchedule = (id) => execute("DELETE FROM exam_schedules WHERE id = ?", [id]);

// ===== Marks =====
const PERF_SCORE = { EE: 4, ME: 3, AE: 2, BE: 1 };

const listMarks = async (schoolId, { exam_id, grade_id, stream_id, student_id, subject_id, status }, session = {}) => {
  const where = ["m.school_id = ?"];
  const params = [schoolId];
  if (exam_id)    { where.push("m.exam_id = ?");    params.push(exam_id); }
  if (student_id) { where.push("m.student_id = ?"); params.push(student_id); }
  if (subject_id) { where.push("m.subject_id = ?"); params.push(subject_id); }
  if (grade_id)   { where.push("st.grade_id = ?");  params.push(grade_id); }
  if (stream_id)  { where.push("st.stream_id = ?"); params.push(stream_id); }
...
const upsertMark = async (data, ctx = {}) => {
  const { actorId = null, actorRole = null, forceAdmin = false, reason = null } = ctx;
  ...
  if (data.score != null) {
    ...
    data.out_of = max;
    data.grading_scale_id = data.grading_scale_id || sub?.grading_scale_id || null;
  }
  ...
  const perfScore = data.performance_level ? PERF_SCORE[data.performance_level] || null : null;

  const existing = await queryOne(
    `SELECT * FROM exam_marks
     WHERE exam_id = ? AND student_id = ? AND subject_name = ?
       AND (competency_id <=> ?)`,
    [data.exam_id, data.student_id, data.subject_name, data.competency_id || null],
  );

  if (existing) {
    if (existing.status === "LOCKED" && !forceAdmin) {
      const e = new Error("Mark is LOCKED — admin override required"); e.statusCode = 409; throw e;
    }
    await execute(
      `UPDATE exam_marks SET
       ...
       WHERE id = ?`,
      [
        data.score ?? null, data.out_of ?? 100,
        gradeInfo.grade, gradeInfo.points,
        data.performance_level || null, perfScore, data.performance_level_id || null,
    ...
    await writeAudit({
      examMarkId: existing.id, examId: data.exam_id, studentId: data.student_id,
      action: "UPDATE",
      oldValue: { score: existing.score, grade: existing.grade, status: existing.status, performance_level: existing.performance_level },
      newValue: { score: data.score, grade: gradeInfo.grade, status: data.status || existing.status, performance_level: data.performance_level },
  ...
  await execute(
    ...
    [
      id, data.school_id, data.academic_year_id || null, data.term_id || null,
      data.exam_id, data.schedule_id || null, data.subject_id || null, data.subject_name,
      data.assessment_type_id || null, data.competency_id || null, data.performance_level_id || null,
      data.score ?? null, data.out_of ?? 100, gradeInfo.grade, gradeInfo.points,
      data.performance_level || null, perfScore,
      data.remarks ?? gradeInfo.remark ?? null, data.recorded_by || actorId, actorId,
      data.status || "DRAFT",
      data.student_id,
    ],
  );
  await writeAudit({
    examMarkId: id, examId: data.exam_id, studentId: data.student_id,
    action: "CREATE",
    newValue: { score: data.score, grade: gradeInfo.grade, status: data.status || "DRAFT" },
    actorId, actorRole, reason,
  });
 ...
 */
const submitDraftMarks = async ({ schoolId, examId, session, actorId, actorRole }) => {
  ...
  for (const d of drafts) {
    await writeAudit({
      examMarkId: d.id, examId, studentId: d.student_id,
      action: "SUBMIT", newValue: { status: "SUBMITTED" }, actorId, actorRole,
    });
  }
...
const deleteMark = async (id, schoolId, ctx = {}) => {
  const existing = await queryOne(
    "SELECT * FROM exam_marks WHERE id = ? AND school_id = ?", [id, schoolId],
  );
  if (!existing) return { deleted: false };
  if (existing.status === "LOCKED" && ctx.actorRole !== "admin") {
    const e = new Error("Cannot delete LOCKED mark"); e.statusCode = 409; throw e;
  }
  await execute("DELETE FROM exam_marks WHERE id = ? AND school_id = ?", [id, schoolId]);
  await writeAudit({
    examMarkId: id, examId: existing.exam_id, studentId: existing.student_id,
    action: "DELETE", oldValue: existing, actorId: ctx.actorId, actorRole: ctx.actorRole, reason: ctx.reason,
  });
  return { deleted: true };
};
...
const listExamSubjects = (examId) =>
  query(`SELECT * FROM exam_subjects WHERE exam_id = ? ORDER BY subject_name`, [examId]);
...
const upsertExamSubject = async (examId, s) => {
  ...
  if (existing) {
    await execute(
      `UPDATE exam_subjects SET subject_id = ?, max_marks = ?, pass_mark = ?, weight = ?, grading_scale_id = ?
       WHERE id = ?`,
      [s.subject_id || null, s.max_marks ?? 100, s.pass_mark ?? null, s.weight ?? 100, s.grading_scale_id || null, existing.id],
    );
  ...
  await execute(
    `INSERT INTO exam_subjects (id, exam_id, subject_id, subject_name, max_marks, pass_mark, weight, grading_scale_id)
     VALUES (?,?,?,?,?,?,?,?)`,
    [id, examId, s.subject_id || null, s.subject_name, s.max_marks ?? 100, s.pass_mark ?? null, s.weight ?? 100, s.grading_scale_id || null],
  );
  return queryOne(`SELECT * FROM exam_subjects WHERE id = ?`, [id]);
};

module.exports = {
  listExams, getExam, createExam, updateExam, deleteExam, transitionExam,
  listSchedules, createSchedule, updateSchedule, deleteSchedule,
  listMarks, upsertMark, bulkUpsertMarks, submitDraftMarks, deleteMark,
  listExamSubjects, upsertExamSubject,
  normalizeStatus,
};
