...
const getFinanceReport = async (schoolId, filters = {}) => {
  ...
  const [payrollSummary] = await db.query(
    `
    SELECT 
        CONCAT(pp.year, '-', LPAD(pp.month, 2, '0')) AS payroll_month,
  ...
  try {
    ...
    if (filters.exam_id) {
      const [marks] = await db.query(
        `
        SELECT em.*, CONCAT(s.first_name, ' ', s.last_name) as student_name, s.admission_number as admission_no
        FROM exam_marks em
