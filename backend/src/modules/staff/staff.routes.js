...
router.put("/departments/:id", async (req, res) => {
  try {
    ...
    await query(
      `UPDATE departments SET ${fields.join(", ")} WHERE id = ? AND school_id = ?`,
      values,
    );
    const row = await queryOne("SELECT * FROM departments WHERE id = ?", [
      req.params.id,
    ]);
    return success(res, row);
  } catch (e) {
...
router.delete("/departments/:id", async (req, res) => {
  try {
    await query("DELETE FROM departments WHERE id = ? AND school_id = ?", [
      req.params.id,
      req.schoolId,
    ]);
...
router.get("/staff/departments", async (req, res, next) => {
  req.url = "/departments";
  return router.handle(req, res, next);
});

router.post("/staff/departments", async (req, res, next) => {
  req.url = "/departments";
  return router.handle(req, res, next);
});
...
router.put("/designations/:id", async (req, res) => {
  try {
    ...
    values.push(req.params.id, req.schoolId);
    await query(
      `UPDATE designations SET ${fields.join(", ")} WHERE id = ? AND school_id = ?`,
      values,
    );
    const row = await queryOne("SELECT * FROM designations WHERE id = ?", [
      req.params.id,
    ]);
    return success(res, row);
  } catch (e) {
...
router.delete("/designations/:id", async (req, res) => {
  try {
    await query("DELETE FROM designations WHERE id = ? AND school_id = ?", [
      req.params.id,
      req.schoolId,
    ]);
    return success(res, { deleted: true });
...
router.get("/staff/designations", async (req, res, next) => {
  req.url = "/designations";
  return router.handle(req, res, next);
});

router.post("/staff/designations", async (req, res, next) => {
  req.url = "/designations";
  return router.handle(req, res, next);
});
