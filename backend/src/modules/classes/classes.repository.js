...
const mapClassRow = (grade, streams = [], studentCount = 0) => ({
  id: grade.id,
  name: grade.name,
...
const findAllClasses = async (schoolId, { limit, offset }) => {
  ...
  const countRows = await query(
    "SELECT COUNT(*) as count FROM grades WHERE school_id = ?",
    [schoolId],
  );
  const rows = await Promise.all(
    grades.map(async (grade) => {
      ...
      const cnt = await query(
        "SELECT COUNT(*) AS c FROM students WHERE school_id = ? AND current_grade_id = ? AND status = 'active'",
        [schoolId, grade.id],
      );
      return mapClassRow(grade, streams, cnt[0]?.c || 0);
    }),
  );
...
const findAllGrades = async (schoolId) => {
  return query(
    `SELECT g.*,
        (SELECT COUNT(*) FROM streams s WHERE s.school_id = g.school_id AND s.grade_id = g.id) AS stream_count,
        (SELECT COUNT(*) FROM students st WHERE st.school_id = g.school_id AND st.current_grade_id = g.id AND st.status = 'active') AS student_count
      FROM grades g WHERE g.school_id = ? ORDER BY g.order_index ASC`,
    [schoolId],
  );
};
...
const findAllStreams = async (schoolId, gradeId) => {
  let sql =
    "SELECT s.*, g.name as grade_name FROM streams s LEFT JOIN grades g ON g.id = s.grade_id WHERE s.school_id = ?";
...
const updateGrade = async (id, schoolId, data) => {
  ...
  if (entries.length === 0)
    return queryOne("SELECT * FROM grades WHERE id = ? AND school_id = ?", [
      id,
      schoolId,
    ]);
  ...
  await query(
    `UPDATE grades SET ${fields.join(", ")} WHERE id = ? AND school_id = ?`,
    values,
  );
  return queryOne("SELECT * FROM grades WHERE id = ?", [id]);
};
...
const updateStream = async (id, schoolId, data) => {
  ...
  values.push(id, schoolId);
  await query(
    `UPDATE streams SET ${fields.join(", ")} WHERE id = ? AND school_id = ?`,
    values,
  );
  return queryOne("SELECT * FROM streams WHERE id = ?", [id]);
};
...
const createSubject = async (data) => {
  const id = uuidv4();
  await query(
    "INSERT INTO subjects (id, school_id, name, code, category, description, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
    [
      id,
      data.school_id,
      data.name,
...
const updateSubject = async (id, schoolId, data) => {
  ...
  values.push(id, schoolId);
  await query(
    `UPDATE subjects SET ${fields.join(", ")} WHERE id = ? AND school_id = ?`,
    values,
  );
  return queryOne("SELECT * FROM subjects WHERE id = ?", [id]);
};
...
const deleteSubject = async (id, schoolId) => {
  await query("DELETE FROM subjects WHERE id = ? AND school_id = ?", [
    id,
    schoolId,
  ]);
  return { deleted: true };
...
const deleteStream = async (id, schoolId) => {
  await query("DELETE FROM streams WHERE id = ? AND school_id = ?", [
    id,
    schoolId,
  ]);
  return { deleted: true };
...
const deleteGrade = async (id, schoolId) => {
  await query("DELETE FROM grades WHERE id = ? AND school_id = ?", [
    id,
    schoolId,
  ]);
  return { deleted: true };
...
const findAllStaff = async (schoolId) => {
  return query(
    "SELECT id, first_name, last_name, employee_number, email, phone, status FROM staff WHERE school_id = ? AND status = ? ORDER BY first_name ASC",
    [schoolId, "active"],
  );
};
...
const updateTimetableEntry = async (id, schoolId, data) => {
  ...
  values.push(id, schoolId);
  await query(
    `UPDATE timetable_entries SET ${fields.join(", ")} WHERE id = ? AND school_id = ?`,
    values,
  );
  return queryOne("SELECT * FROM timetable_entries WHERE id = ?", [id]);
};
...
const deleteTimetableEntry = async (id, schoolId) => {
  await query("DELETE FROM timetable_entries WHERE id = ? AND school_id = ?", [
    id,
    schoolId,
  ]);
  return { deleted: true };
