...
import { Check, X, Trash2, Plus, Package } from "lucide-react";
...
export default function InKindPayments() {
  ...
  const [tab, setTab] = useState<"supplier_offset" | "parent_goods">("supplier_offset");
  ...
  const approveMut = useMutation({
    ...
    onSuccess: () => { toast.success("Approved & posted"); qc.invalidateQueries({ queryKey: ["in-kind"] }); },
    onError: (e: any) => toast.error(e.message),
  });
  const rejectMut = useMutation({
    mutationFn: (id: string) => api.post(`/in-kind-payments/${id}/reject`, { reason: "Rejected" }),
    onSuccess: () => { toast.success("Rejected"); qc.invalidateQueries({ queryKey: ["in-kind"] }); },
  });
  const delMut = useMutation({
    mutationFn: (id: string) => api.delete(`/in-kind-payments/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["in-kind"] }); },
    onError: (e: any) => toast.error(e.message),
  });
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Record goods/services as fee payments</p>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button><Plus className="h-4 w-4 mr-2" />New In-Kind Record</Button>
            </DialogTrigger>
            <DialogContent className="max-w-xl">
              <DialogHeader><DialogTitle>Record In-Kind Payment</DialogTitle></DialogHeader>
              <InKindForm
                kind={tab} suppliers={suppliers} students={students}
                onSave={(d: any) => createMut.mutate({ ...d, kind: tab })}
                onClose={() => setOpen(false)} saving={createMut.isPending}
              />
            </DialogContent>
          </Dialog>
        </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>{tab === "supplier_offset" ? "Supplier" : "Student"}</TableHead>
                  ...
                  <TableBody>
                    {records.length === 0 ? (
                      <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-6">No records</TableCell></TableRow>
                    ) : records.map((r: any) => (
                      <TableRow key={r.id}>
                        <TableCell>{formatDate(r.created_at)}</TableCell>
                        <TableCell>{tab === "supplier_offset" ? r.supplier_name : r.student_name}</TableCell>
                        <TableCell className="max-w-[260px] truncate">{r.goods_description}</TableCell>
                        <TableCell>{r.quantity} {r.unit || ""}</TableCell>
                        <TableCell className="text-right">{formatKES(r.assessed_value)}</TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          {r.approval_status === "pending" && (
                            <div className="flex gap-1 justify-end">
                              ...
                              <Button size="sm" variant="outline" onClick={() => rejectMut.mutate(r.id)}>
                                <X className="h-4 w-4" />
                              </Button>
...
function InKindForm({ kind, suppliers, students, onSave, onClose, saving }: any) {
  ...
  const submit = () => {
    ...
    if (kind === "supplier_offset" && !supplierId) return toast.error("Select supplier");
    ...
    onSave({
      ...
      unit, assessed_value: Number(value),
      reference, notes,
    });
  };
    ...
    <div className="grid gap-4 py-2">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="space-y-2"><Label>Unit</Label>
          <Input value={unit} onChange={(e) => setUnit(e.target.value)} placeholder="bags, hrs..." /></div>
        <div className="space-y-2"><Label>Assessed Value (KES) *</Label>
