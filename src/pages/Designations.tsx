...
export default function Designations() {
  ...
  const staffCounts = (staffList as any[]).reduce((acc: Record<string, number>, s: any) => {
    if (s.designation_id) acc[s.designation_id] = (acc[s.designation_id] || 0) + 1;
    return acc;
  }, {});

  const save = useMutation({
    mutationFn: () => form.id
      ? api.put(`/designations/${form.id}`, form)
      : api.post("/designations", { ...form, school_id: schoolId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["designations"] });
      setOpen(false);
      setForm({ name: "", description: "" });
      toast({ title: form.id ? "Designation updated" : "Designation added" });
    },
    onError: (err: any) => toast({ title: "Error", description: err.message, variant: "destructive" }),
  });

  const openEdit = (d?: any) => {
    setForm(d ? { id: d.id, name: d.name, description: d.description || "", is_active: !!d.is_active } : { name: "", description: "" });
    setOpen(true);
  };
  ...
  return (
    <DashboardLayout title="Designations" subtitle="Staff roles and job titles">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{form.id ? "Edit" : "Add"} Designation</DialogTitle></DialogHeader>
