...
import { useTeacherTimetable, useStaff } from "@/hooks/useClasses";
...
const TeacherTimetable = () => {
  ...
  const { data: entries = [], isLoading } = useTeacherTimetable(selectedTeacher || undefined);

  const getEntry = (day: string, period: number) => entries.find((t) => t.day === day && t.period === period);
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card className="bg-warning/5 border-warning/10"><CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-warning">{subjectCount}</p><p className="text-xs text-muted-foreground">Subjects Assigned</p>
          </CardContent></Card>
        </div>
