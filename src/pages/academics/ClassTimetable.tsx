        ...
        <div className="flex items-center justify-between">
          ...
          {perms["classes:create"] && (
            ...
            >
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{form.id ? "Edit" : "Add"} period</DialogTitle>
                </DialogHeader>
                <div className="grid grid-cols-2 gap-3 py-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      value={form.kind}
                      onValueChange={(v) =>
                        setForm((p) => ({ ...p, kind: v as PeriodKind }))
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      value={form.curriculum_type || "CBC"}
                      onValueChange={(v) =>
                        setForm((p) => ({
                          ...p,
                          curriculum_type: v as "CBC" | "844",
                        }))
                      }
                    >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        <SelectItem value="CBC">CBC / CBE</SelectItem>
      ...
      <CardContent className="p-0">
        {isLoading ? (
          ...
          <p className="text-center text-sm text-muted-foreground py-8">
            No periods configured yet. Add lessons, breaks, lunch, and assembly
            slots.
          </p>
        ) : (
          <Table>
            ...
            <TableBody>
              {periods.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <Badge className={kindBadge(p.kind)} variant="outline">
                      {p.kind}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline">
                      {p.curriculum_type || "CBC"}
                    </Badge>
                  </TableCell>
  ...
  const handleSave = () => {
    ...
    const items = merged
      ...
      .map((m) => ({
        ...
        lessons_per_week: m.lessons_per_week,
      }));
  ...
  return (
    <Card>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex gap-2">
            ...
            {perms["classes:update"] && (
              <Button
                size="sm"
                disabled={!gradeId || bulkSave.isPending}
                onClick={handleSave}
              >
                Save
              </Button>
      ...
      <CardContent className="p-0">
        {!gradeId ? (
          <p className="text-center text-sm text-muted-foreground py-8">
            Choose a class to configure its weekly lesson load.
          </p>
        ) : isLoading ? (
          <div className="p-6">
            <Skeleton className="h-40 w-full" />
                ...
                <TableRow key={m.subject_id}>
                  ...
                  <TableCell>
                    <Input
                      ...
                      min={0}
                      max={20}
                      value={m.lessons_per_week}
                      onChange={(e) =>
                        setDraft((d) => ({
                          ...d,
                          [m.subject_id]: Math.max(
                            0,
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Pick the classes to generate. Existing entries for affected streams
            will be replaced. Uses period setup, lesson requirements, and
          ...
          <div className="flex gap-2">
            {perms["classes:create"] && (
              <Button
                disabled={!selected.length || gen.isPending}
                onClick={() =>
                  gen.mutate({ grade_ids: selected, replace: true })
                }
              >
            ...
            <Button
              ...
              onClick={() => setSelected(grades.map((g) => g.id))}
            >
              Select all
            </Button>
          ...
          {gen.data && (
            <Alert>
              ...
              <AlertDescription className="text-sm">
                Assigned{" "}
                {(gen.data as any)?.data?.assigned ??
                  (gen.data as any).assigned}{" "}
                lessons, skipped{" "}
                {(gen.data as any)?.data?.skipped ?? (gen.data as any).skipped}.
                {(
                  (gen.data as any)?.data?.warnings ||
                  (gen.data as any).warnings ||
                  []
                ).length > 0 && (
          ...
          (clashes.teacherClashes.length === 0 &&
            ...
            <div className="space-y-2 text-sm">
              {clashes.teacherClashes.length > 0 && (
                <div>
                  <p className="font-medium">
                    Teacher double-bookings: {clashes.teacherClashes.length}
                  </p>
                  <ul className="text-xs text-muted-foreground pl-4 list-disc">
                    {clashes.teacherClashes.slice(0, 10).map((c, i) => (
                      <li key={i}>
                        Teacher {c.teacher_id.slice(0, 8)} — {c.day} period{" "}
                        {c.period} ({c.c} lessons)
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {clashes.classClashes.length > 0 && (
                <div>
                  <p className="font-medium">
                    Class double-bookings: {clashes.classClashes.length}
                  </p>
                </div>
              )}
            </div>
      ...
      <Table>
        ...
        <TableBody>
          {periods.map((p) => (
            <TableRow
              key={p.id}
              className={p.kind !== "lesson" ? "bg-muted/30" : ""}
            >
...
function ClassView() {
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `timetable-${streamId}.csv`;
    a.click();
...
const ClassTimetable = () => {
  ...
  return (
    <DashboardLayout
      title="Timetable"
      subtitle="Configure periods, lesson loads, auto-generate, and view class/teacher schedules"
    >
