  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Distribute one sponsor payment across many students
            </p>
          </div>
          <HistoricalReadOnlyGate>
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>Bulk Sponsorship Wizard</DialogTitle>
                </DialogHeader>
                <BulkWizard
                  onSaved={() => {
                    qc.invalidateQueries({ queryKey: ["bulk-payments"] });
        ...
        <Card>
          <CardHeader>
            <CardTitle>Bulk Payments</CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                  ...
                  bulks.map((b: any) => (
                    <TableRow key={b.id}>
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            b.status === "committed" ? "default" : "secondary"
                          }
                        >
                          {b.status}
                        </Badge>
        ...
        <Dialog open={!!viewId} onOpenChange={(o) => !o && setViewId(null)}>
          <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Bulk Allocations</DialogTitle>
            </DialogHeader>
            {detail && (
              <div className="space-y-3">
                <div className="text-sm">
                  <div>
                    <b>Sponsor:</b> {detail.sponsor_name}
                  </div>
                  <div>
                    <b>Total:</b> {formatKES(detail.total_amount)}
                  </div>
                  <div>
    ...
    <div className="space-y-4">
      <div className="flex gap-2 text-sm">
        {["Sponsor", "Students", "Distribute", "Review"].map((label, i) => (
          <Badge key={i} variant={step === i + 1 ? "default" : "outline"}>
            {i + 1}. {label}
          </Badge>
        ))}
      </div>
      ...
      {step === 3 && (
        <div className="space-y-3">
          ...
          <div className="text-right text-sm">
            Allocated: <b>{formatKES(allocTotal)}</b> of{" "}
      ...
      {step === 4 && (
        <div className="space-y-2 text-sm">
          ...
          <div>
            <b>Total:</b> {formatKES(Number(total) || 0)}
          </div>
          <div>
            <b>Students:</b> {selectedIds.length}
          </div>
          <div>
            <b>Allocated:</b> {formatKES(allocTotal)}
          </div>
          <div className="text-muted-foreground">
            Click "Save Draft" — you can review and commit later.
          </div>
        </div>
      )}
      ...
      <DialogFooter className="gap-2">
        ...
        {step < 4 ? (
          <Button
            onClick={() => {
              if (step === 1 && (!sponsor || !total))
                return toast.error("Sponsor and total required");
