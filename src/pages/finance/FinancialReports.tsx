...
function DateRange({ from, to, setFrom, setTo }: any) {
  ...
  const thisFy = fiscalYearRange(startMonth, 0);
  const lastFy = fiscalYearRange(startMonth, -1);
  return (
...
export function FinancialYearCard() {
  ...
  const current = String(data?.fiscal_year_start_month ?? 7);
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Financial Year</CardTitle>
        <CardDescription>
          Sets the reporting year used by report presets. Kenyan schools
          typically run July 1 – June 30.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-wrap items-end gap-3">
        <div className="w-56">
          ...
          <Select
            value={current}
            onValueChange={(v) =>
              update.mutate({ fiscal_year_start_month: Number(v) })
            }
          >
            <SelectTrigger>
              <SelectValue />
        ...
        <Badge variant="outline">
          Current: {fiscalYearRange(Number(current)).label}
        </Badge>
      </CardContent>
    </Card>
  );
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle
              className={`text-2xl ${Number(data?.totals.surplus) < 0 ? "text-destructive" : "text-emerald-600"}`}
            >
  ...
  const Section = ({ title, rows, total }: any) => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Total: {fmt(total)}</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            <Card>
              <CardHeader>
                ...
                <CardTitle>{fmt(data.totals.opening_balance)}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader>
                <CardDescription>Invoiced</CardDescription>
                <CardTitle>{fmt(data.totals.invoiced)}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader>
                <CardDescription>Paid</CardDescription>
                <CardTitle>{fmt(data.totals.paid)}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader>
                <CardDescription>Balance</CardDescription>
                <CardTitle>{fmt(data.totals.balance)}</CardTitle>
              </CardHeader>
            </Card>
          </div>
          <Card>
                ...
                <TableBody>
                  {data.vouchers.map((v: any) => (
                    <TableRow key={v.id}>
                      ...
                      <TableCell>{v.voucher_date?.slice(0, 10)}</TableCell>
                      <TableCell>
...
function ApAgingTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>AP Aging by Supplier</CardTitle>
          <CardDescription>
            Total outstanding: {fmt(data?.totals.total)}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {(data?.suppliers || []).map((s: any) => (
                <TableRow key={s.supplier_id}>
                  ...
                  <TableCell className="text-right">{fmt(s.current)}</TableCell>
                  <TableCell className="text-right">{fmt(s.d1_30)}</TableCell>
                  <TableCell className="text-right">{fmt(s.d31_60)}</TableCell>
                  <TableCell className="text-right">{fmt(s.d61_90)}</TableCell>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Card>
          <CardHeader>
            ...
            <CardTitle>{fmt(data?.totals.total)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle>{fmt(data?.totals.approved)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>Draft</CardDescription>
            <CardTitle>{fmt(data?.totals.draft)}</CardTitle>
          </CardHeader>
        </Card>
      </div>
      <Card>
            ...
            <TableBody>
              {(data?.rows || []).map((v: any) => (
                <TableRow key={v.id}>
                  ...
                  <TableCell>{v.bank_account_name || "—"}</TableCell>
                  <TableCell>
                    <Badge variant="outline">{v.status}</Badge>
                  </TableCell>
                  <TableCell className="text-right">{fmt(v.amount)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardHeader>
            ...
            <CardTitle>{data?.totals.count || 0}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle>{fmt(data?.totals.cancelled)}</CardTitle>
          </CardHeader>
        </Card>
      </div>
      <Card>
            ...
            <TableBody>
              {(data?.rows || []).map((r: any) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>{r.receipt_date?.slice(0, 10)}</TableCell>
                  <TableCell>{r.admission_number}</TableCell>
                  <TableCell>{r.student_name}</TableCell>
                  <TableCell>{r.payment_method}</TableCell>
                  <TableCell>
                    <Badge variant="outline">{r.status || "issued"}</Badge>
                  </TableCell>
            ...
            <TableBody>
              {(data?.rows || []).map((p: any) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>{p.order_date?.slice(0, 10)}</TableCell>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardHeader>
            ...
            <CardTitle>{fmt(data?.totals.expected)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>Received</CardDescription>
            <CardTitle>{fmt(data?.totals.received)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>Pending</CardDescription>
            <CardTitle>{fmt(data?.totals.pending)}</CardTitle>
          </CardHeader>
        </Card>
      </div>
      <Card>
            ...
            <TableBody>
              {(data?.rows || []).map((c: any) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell className="text-right">
                    {fmt(c.received_amount)}
                  </TableCell>
                  <TableCell>
                    {c.tranches_received}/{c.tranches_total}
                  </TableCell>
                  <TableCell>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            <Card>
              <CardHeader>
                ...
                <CardTitle>{fmt(data.totals.budgeted)}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader>
                <CardDescription>Actual</CardDescription>
                <CardTitle>{fmt(data.totals.actual)}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader>
            ...
            <Card>
              <CardHeader>
                <CardDescription>Utilisation</CardDescription>
                <CardTitle>{data.totals.utilisation}%</CardTitle>
              </CardHeader>
            </Card>
          </div>
          <Card>
              ...
              <Table>
                ...
                <TableBody>
                  {data.lines.map((l: any) => (
                    <TableRow key={l.id}>
                      <TableCell>{l.vote_head_name}</TableCell>
                      <TableCell className="text-right">
                        {fmt(l.budgeted)}
                      </TableCell>
                      <TableCell className="text-right">
                      ...
                      <TableCell
                        className={`text-right ${l.variance < 0 ? "text-destructive" : ""}`}
                      >
                        {fmt(l.variance)}
                      </TableCell>
                      <TableCell className="text-right">
        ...
        <CardContent className="pt-6">
          <Table>
            ...
            <TableBody>
              {(data?.rows || []).map((r: any) => (
                <TableRow key={r.id}>
                  <TableCell>{r.statement_date?.slice(0, 10)}</TableCell>
                  ...
                  <TableCell>{r.unmatched_count}</TableCell>
                  <TableCell>
        ...
        <div className="space-y-4 p-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Income &amp; Expenditure, Balance Sheet, supplier statements,
              registers, capitation, budget-vs-actual and bank reconciliation —
              all derived live from the general ledger.
            </p>
          </div>
