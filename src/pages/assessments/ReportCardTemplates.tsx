...
function useUpdateSchoolProfile() {
  ...
  return useMutation({
    mutationFn: (data: Partial<SchoolProfile>) => api.put("/schools/profile", data),
    onSuccess: () => {
...
export default function ReportCardTemplates() {
  ...
  // Template dialog
  const [editing, setEditing] = useState<Partial<ReportCardTemplate> | null>(null);
    ...
    >
                  ...
                  {isLoading ? (
                    ...
                    <TableRow key={t.id}>
                      <TableCell className="font-medium">
                        {t.name}{" "}
                        {t.is_default && <Badge variant="secondary" className="ml-1">default</Badge>}
                      </TableCell>
                      <TableCell><Badge variant="outline">{t.kind}</Badge></TableCell>
                      <TableCell className="text-sm">
                        <div className="font-medium">{t.header_title || "—"}</div>
                        <div className="text-xs text-muted-foreground">{t.header_subtitle || ""}</div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-wrap gap-1 text-[10px]">
                          {t.show_position && <Badge variant="outline">Position</Badge>}
                          {t.show_band && <Badge variant="outline">Band</Badge>}
                          {t.show_competencies && <Badge variant="outline">Competencies</Badge>}
                          {t.show_teacher_remarks && <Badge variant="outline">Teacher</Badge>}
                          {t.show_principal_remarks && <Badge variant="outline">Principal</Badge>}
                        </div>
                      </TableCell>
            ...
            <CardContent className="space-y-4 max-w-3xl">
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input value={school?.name || currentSchool?.name || ""} disabled />
                </div>
                <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                in a future update — for now they appear as signature lines on the PDF.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <DialogContent className="max-w-2xl">
            ...
            <div className="grid gap-4 py-2">
              <div className="grid sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="e.g. CBC End-Term Report"
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setEditing({ ...editing, kind: v as any })}
                  >
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setEditing({ ...editing, header_title: e.target.value })}
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setEditing({ ...editing, header_subtitle: e.target.value })}
              ...
              <div className="grid sm:grid-cols-2 gap-2 border rounded-md p-3">
                {[
                  ...
                  ["show_band", "Show CBC performance band"],
                  ...
                  <label key={key} className="flex items-center justify-between text-sm">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(v) => setEditing({ ...editing, [key]: v } as any)}
                    />
                  </label>
                ))}
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={onSubmitTemplate} disabled={save.isPending}>
              <Save className="h-4 w-4 mr-1.5" /> {save.isPending ? "Saving…" : "Save Template"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!previewing} onOpenChange={(o) => { if (!o) setPreviewing(null); }}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>Layout preview — {previewing?.name}</DialogTitle></DialogHeader>
          {previewing && (
            <div className="border rounded-md p-6 bg-card text-sm space-y-3">
              <div className="flex items-center gap-3 border-b pb-3">
                {logoUrl && <img src={logoUrl} alt="" className="h-12 w-12 object-contain" />}
                <div>
                  <div className="text-lg font-bold uppercase">{school?.name || "Your School"}</div>
                  {address && <div className="text-xs text-muted-foreground">{address}</div>}
                </div>
              </div>
              <div className="text-center">
                <div className="font-bold text-base uppercase">{previewing.header_title || "Report Card"}</div>
                {previewing.header_subtitle && (
                  <div className="text-xs italic text-muted-foreground">{previewing.header_subtitle}</div>
                )}
              </div>
              <div className="grid grid-cols-3 text-xs gap-2 border-y py-2">
                <div><span className="text-muted-foreground">Student:</span> Jane Doe</div>
                <div><span className="text-muted-foreground">Class:</span> Grade 5 · East</div>
                <div><span className="text-muted-foreground">Term:</span> Term 2 / 2026</div>
              </div>
              <Table>
                ...
                <TableBody>
                  {[
                    ...
                    <TableRow key={s as string}>
                      ...
                      {previewing.show_band && <TableCell><Badge variant="outline">EE</Badge></TableCell>}
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
              <div className="grid grid-cols-2 gap-3 text-xs">
                {previewing.show_position && <div><span className="text-muted-foreground">Position:</span> 3 / 42</div>}
                {previewing.show_band && <div><span className="text-muted-foreground">Overall band:</span> Exceeding Expectation</div>}
              </div>
              {previewing.show_teacher_remarks && (
                <div className="text-xs"><span className="text-muted-foreground">Class teacher:</span> Excellent term, keep it up. ____________</div>
              )}
              {previewing.show_principal_remarks && (
                <div className="text-xs"><span className="text-muted-foreground">Head teacher:</span> A commendable performance. ____________</div>
              )}
            </div>
          )}
        </DialogContent>
