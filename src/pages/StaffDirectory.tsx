...
// Roles are now fetched dynamically from the database
...
export default function StaffDirectory() {
  ...
  const openEdit = (s: any) => {
    setForm({
      ...
      role: s.roles?.split(',')[0]?.trim() || "teacher",
  ...
  const headlineStats = [
    ...
    {
      label: "Teachers",
      value: staffList.filter((s: any) => s.roles?.includes("teacher")).length,
    },
    {
      label: "Departments",
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            All staff members across all roles.
          </p>
          <Dialog
            open={isAddOpen}
            onOpenChange={(o) => {
                    ...
                    <TableRow key={s.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className="capitalize">
                          {s.roles?.split(",").map((r: string) => r.trim().replace(/_/g, " ")).join(", ")}
                        </Badge>
                      </TableCell>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      ...
                      {viewStaff.roles?.split(",").map((r: string) => r.trim().replace(/_/g, " ")).join(", ")}
                    </p>
                  </div>
                </div>
