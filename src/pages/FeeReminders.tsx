...
const FeeReminders = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      try {
        ...
        setGrades(
          (g?.data || g || []).map((x: any) => ({ id: x.id, name: x.name })),
        );
      } catch {
        /* ignore */
      }
  ...
  const filtered = useMemo(() => {
    return students.filter((s) => {
      ...
      const matchesGrade =
        gradeFilter === "all" || s.grade_name === gradeFilter;
      const matchesBalance =
        balanceFilter === "all" ||
        (balanceFilter === "overdue" && s.overdue_count > 0) ||
        (balanceFilter === "high" && s.total_balance >= 10000);
      const p = s.percent_paid;
      const matchesPercent =
        percentFilter === "all" ||
        (percentFilter === "0" && p === 0) ||
        (percentFilter === "lt10" && p > 0 && p < 10) ||
        (percentFilter === "lt25" && p >= 10 && p < 25) ||
        (percentFilter === "lt50" && p >= 25 && p < 50) ||
        (percentFilter === "lt75" && p >= 50 && p < 75) ||
        (percentFilter === "lt90" && p >= 75 && p < 90) ||
        (percentFilter === "gte90" && p >= 90);
      return matchesSearch && matchesGrade && matchesBalance && matchesPercent;
    });
  }, [students, search, gradeFilter, balanceFilter, percentFilter]);
  ...
  const handleSendReminders = async () => {
    ...
    try {
      const ids = selectedStudents
        ...
        .map((s) => s.id);
      if (ids.length === 0) {
        toast.error("No selected students have a valid parent phone");
        setSending(false);
        return;
      }
      const res = await api.post<any>("/finance/fee-reminders/send", {
        student_ids: ids,
        message_template: messageTemplate,
      });
      const d = res?.data || res;
      toast.success(
        `Sent ${d?.sent ?? 0}/${d?.total ?? ids.length} reminders${d?.failed ? `, ${d.failed} failed` : ""}`,
      );
      setSendDialog(false);
      setSelectedIds(new Set());
      fetchHistory();
  ...
  return (
    <DashboardLayout
      title="Fee Reminders"
      subtitle="Send fee payment reminders to parents"
    >
      <motion.div
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <Select value={percentFilter} onValueChange={setPercentFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="0">0% paid</SelectItem>
                    <SelectItem value="lt10">Below 10%</SelectItem>
                    <SelectItem value="lt25">10% – 25%</SelectItem>
                    <SelectItem value="lt50">25% – 50%</SelectItem>
                    <SelectItem value="lt75">50% – 75%</SelectItem>
                    <SelectItem value="lt90">75% – 90%</SelectItem>
                    ...
                    filtered.map((s) => (
                      ...
                      >
                        ...
                        <TableCell className="text-right font-bold text-destructive">
                          {formatKES(s.total_balance)}
                        </TableCell>
                        <TableCell className="text-center">
                          <Badge variant="secondary">{s.fee_count}</Badge>
                        </TableCell>
                        <TableCell className="text-center">
                          {s.overdue_count > 0 ? (
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4">
            <div className="p-3 rounded-lg bg-muted/50">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold">
                  {
                    selectedStudents.filter((s) => s.parent_phone !== "N/A")
                      .length
                  }
                </span>
              </div>
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-destructive">
                  {formatKES(
                    selectedStudents.reduce((s, st) => s + st.total_balance, 0),
                  )}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {"{admission_no}"}, {"{balance}"}, {"{school_name}"}
              </p>
            </div>
            ...
            <div className="p-3 rounded-lg bg-warning/5 border border-warning/20 flex items-start gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                SMS messages will be queued for delivery. Actual sending
                requires an SMS provider (e.g., Africa's Talking) to be
                configured.
              </p>
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            >
              {sending
                ? "Sending..."
                : `Send ${selectedStudents.filter((s) => s.parent_phone !== "N/A").length} Reminders`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
