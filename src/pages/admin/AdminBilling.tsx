...
export default function AdminBilling() {
  const [tab, setTab] = useState<"invoices" | "subscriptions" | "revenue">(
    "invoices",
  );
  ...
  const exportCsv = () => {
    ...
    a.download = `chuo-invoices-${Date.now()}.csv`;
    a.click();
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            All invoices, subscriptions and revenue across CHUO.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-black">{fmt(totals.paid)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-black">{fmt(totals.pending)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
        ...
        <div className="inline-flex rounded-lg border p-1 bg-muted/40">
          {(["invoices", "subscriptions", "revenue"] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-4 py-1.5 text-sm font-semibold rounded-md ${tab === t ? "bg-background shadow" : "text-muted-foreground"}`}
            >
              {t}
            </button>
          ))}
        ...
        {tab !== "revenue" && (
          ...
          >
            ...
            <SelectContent>
              ...
              {tab === "invoices"
                ? ["pending", "paid", "void", "failed"].map((s) => (
                    <SelectItem key={s} value={s}>
                      {s}
                    </SelectItem>
                  ))
                : ["trial", "active", "past_due", "locked", "cancelled"].map(
                    (s) => (
                      <SelectItem key={s} value={s}>
                        {s}
                      </SelectItem>
