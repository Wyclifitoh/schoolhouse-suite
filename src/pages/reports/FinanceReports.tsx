...
const FinanceReports = () => {
  ...
  const { data: report, isLoading } = useFinanceReportData({
    classId: classFilter === "all" ? "" : classFilter,
    startDate,
    endDate,
  });

  const { data: paymentsReport, isLoading: paymentsLoading } =
    usePaymentsReportData({
      startDate,
      endDate,
    });
  ...
  const balanceFees = safeArray(report?.balanceFees);
  const feeStatements = safeArray(report?.feeStatements);
  const dailyCollections = safeArray(report?.dailyCollections);
  const incomeByCategory = safeArray(report?.incomeByCategory);
  const expenseReport = safeArray(report?.expenses);
  const payrollSummary = safeArray(report?.payrollSummary);
  const mpesaPayments = safeArray(paymentsReport?.mpesaPayments);
  const allPayments = safeArray(paymentsReport?.payments);
          ...
          <Card>
            ...
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <EmptyState message="No balance fee data found" />
              ) : (
                <Table>
                  <TableHeader>
          ...
          <Card>
            ...
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <EmptyState message="No fee statement data available" />
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/50">
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <EmptyState message="No collection data for the selected period" />
              ) : (
                <>
            ...
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {payrollSummary.map((p: any, i: number) => (
                      <TableRow key={i}>
                        ...
                        <TableCell className="text-right text-success">
                          {formatKES(p.allowances)}
                        </TableCell>
                        <TableCell className="text-right text-destructive">
                          {formatKES(p.deductions)}
                        </TableCell>
          ...
          <Card>
            ...
            <CardContent className="p-0">
              {paymentsLoading ? (
                ...
                <EmptyState message="No M-Pesa payment records found" />
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/50">
