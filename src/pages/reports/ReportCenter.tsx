          ...
          <section>
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {REPORT_CATEGORIES.map((c) => {
                const count = reportsByCategory(c.key).length;
                return (
                  <Card
                    key={c.key}
                    className="group flex cursor-pointer flex-col transition-all hover:-translate-y-0.5 hover:shadow-lg"
                    onClick={() => navigate(`/reports/category/${c.key}`)}
                  >
