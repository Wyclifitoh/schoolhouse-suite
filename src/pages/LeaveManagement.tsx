...
export default function LeaveManagement() {
  ...
  const [typeForm, setTypeForm] = useState({
    name: "", code: "", max_days: "12", is_paid: true,
  });
  const [allocForm, setAllocForm] = useState({
    staff_id: "", leave_type_id: "", year: String(currentYear), allocated_days: "",
  });
  ...
  const { data: balances = [] } = useQuery({
    queryKey: ["leave-balances", schoolId, currentYear],
    queryFn: () => api.get<any[]>(`/leaves/balances?year=${currentYear}`),
    enabled: !!schoolId,
  });

  const addTypeMutation = useMutation({
    mutationFn: () => api.post("/leaves/types", { ...typeForm, school_id: schoolId }),
    onSuccess: () => {
  ...
  const applyMutation = useMutation({
    mutationFn: () => api.post("/leaves/applications", { ...form, school_id: schoolId }),
    onSuccess: () => {
  ...
  const actionMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      api.put(`/leaves/applications/${id}/status`, { status, approved_by: user?.id }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["leave-applications"] });
  ...
  const allocateMutation = useMutation({
    mutationFn: () =>
      api.post("/leaves/balances", {
        ...allocForm,
        year: Number(allocForm.year),
        allocated_days: Number(allocForm.allocated_days),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["leave-balances"] });
      setIsAllocOpen(false);
      setAllocForm({ staff_id: "", leave_type_id: "", year: String(currentYear), allocated_days: "" });
      toast({ title: "Leave balance allocated" });
    },
    onError: (err: any) =>
      toast({ title: "Error", description: err.message, variant: "destructive" }),
  });
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2 flex-wrap">
              ...
              <DialogContent>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      <Label>Code</Label>
                      <Input placeholder="SL" value={typeForm.code}
                        onChange={(e) => setTypeForm((p) => ({ ...p, code: e.target.value }))} />
                    </div>
                    <div className="grid gap-2">
                      <Label>Max Days per Year</Label>
                      <Input type="number" value={typeForm.max_days}
                        onChange={(e) => setTypeForm((p) => ({ ...p, max_days: e.target.value }))} />
                    </div>
                  </div>
            ...
            <Dialog open={isAllocOpen} onOpenChange={setIsAllocOpen}>
              ...
              <DialogContent>
                <DialogHeader><DialogTitle>Allocate Leave Balance</DialogTitle></DialogHeader>
                <div className="space-y-4 pt-4">
                  <div>
                    <Label>Staff Member</Label>
                    <Select value={allocForm.staff_id}
                      onValueChange={(v) => setAllocForm((p) => ({ ...p, staff_id: v }))}>
                  ...
                  <div>
                    ...
                    <Select value={allocForm.leave_type_id}
                      onValueChange={(v) => setAllocForm((p) => ({ ...p, leave_type_id: v }))}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div><Label>Year</Label>
                      <Input type="number" value={allocForm.year}
                        onChange={(e) => setAllocForm((p) => ({ ...p, year: e.target.value }))} /></div>
                    <div><Label>Allocated Days</Label>
                      <Input type="number" value={allocForm.allocated_days}
                        onChange={(e) => setAllocForm((p) => ({ ...p, allocated_days: e.target.value }))} /></div>
                  </div>
                  <Button className="w-full"
                    onClick={() => allocateMutation.mutate()}
                    disabled={!allocForm.staff_id || !allocForm.leave_type_id || !allocForm.allocated_days || allocateMutation.isPending}>
                    Save Allocation
                  </Button>
                </div>
              </DialogContent>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <div>
                    ...
                    <Select value={form.leave_type_id}
                      onValueChange={(v) => setForm((p) => ({ ...p, leave_type_id: v }))}>
                      <SelectTrigger><SelectValue placeholder="Select type" /></SelectTrigger>
                      <SelectContent>
                        {leaveTypes.map((t: any) => (
                          <SelectItem key={t.id} value={t.id}>{t.name} ({t.max_days} days)</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><Label>Start Date</Label>
                      <Input type="date" value={form.start_date}
                        onChange={(e) => setForm((p) => ({ ...p, start_date: e.target.value }))} /></div>
                    <div><Label>End Date</Label>
                      <Input type="date" value={form.end_date}
                        onChange={(e) => setForm((p) => ({ ...p, end_date: e.target.value }))} /></div>
                  </div>
                  <div>
                    <Label>Reason</Label>
        ...
        <Tabs defaultValue="applications">
          <TabsList>
            ...
            <TabsTrigger value="balances">Balances ({currentYear})</TabsTrigger>
          </TabsList>
                    ...
                    {isLoading ? (
                      ...
                      leaveApplications.map((leave: any) => (
                        <TableRow key={leave.id}>
                          ...
                          <TableCell className="text-sm">{formatDate(leave.start_date)} - {formatDate(leave.end_date)}</TableCell>
          ...
          <TabsContent value="balances" className="space-y-4 mt-4">
            <Card>
              <CardHeader><CardTitle>Leave Balances — {currentYear}</CardTitle></CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {balances.length === 0 ? (
                      <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No balances allocated yet. Click "Allocate Balance" to add.</TableCell></TableRow>
                    ) : (
                      balances.map((b: any) => (
                        <TableRow key={b.id}>
                          <TableCell className="font-medium">{b.staff_name}</TableCell>
                          <TableCell>{b.leave_type_name}</TableCell>
                          <TableCell className="text-right">{b.allocated_days}</TableCell>
                          <TableCell className="text-right">{b.used_days}</TableCell>
                          <TableCell className="text-right">
                            <Badge variant={Number(b.remaining_days) > 0 ? "default" : "destructive"}>
                              {b.remaining_days}
                            </Badge>
                          </TableCell>
                        </TableRow>
