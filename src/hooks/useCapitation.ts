...
export const useCapitationAllocations = (params: {
  ...
  useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get<any>(`/capitation/allocations?${qs}`);
      // api.get already returns the payload envelope's `data` field.
      const payload = Array.isArray(res?.data) ? res : res?.data;
      return {
        data: (payload?.data ?? []) as CapitationStudentAllocation[],
...
export const useCapitationMutations = (id?: string) => {
  ...
  return {
    create: useMutation({
      ...
      onSuccess: () => {
        invalidate();
        toast({ title: "Capitation created" });
      },
      onError: onErr,
    }),
    update: useMutation({
    ...
    activate: useMutation({
      mutationFn: (cid: string) =>
        api.post<any>(`/capitation/${cid}/activate`, {}).then(unwrap),
      onSuccess: () => {
        invalidate();
    ...
    close: useMutation({
      mutationFn: (cid: string) =>
        api.post<any>(`/capitation/${cid}/close`, {}).then(unwrap),
      onSuccess: () => {
        invalidate();
        toast({ title: "Closed" });
      },
    ...
    remove: useMutation({
      mutationFn: (cid: string) =>
        api.delete<any>(`/capitation/${cid}`).then(unwrap),
      onSuccess: () => {
        invalidate();
        toast({ title: "Deleted" });
      },
    ...
    receiveTranche: useMutation({
      mutationFn: ({
        ...
        api
          .post<any>(`/capitation/${id}/tranches/${tid}/receive`, body)
          .then(unwrap),
      onSuccess: () => {
        invalidate();
    ...
    reallocate: useMutation({
      mutationFn: (tid: string) =>
        api
          .post<any>(`/capitation/${id}/tranches/${tid}/allocate`, {})
          .then(unwrap),
      onSuccess: (r: any) => {
        invalidate();
        qc.invalidateQueries({ queryKey: ["capitation-allocations"] });
