...
type Draft = Record<string, { papers: Record<string, string>; remarks?: string }>;
...
export function PaperMarksGrid({
  ...
  const rows = useMemo(() => {
    return roster.students.map((s) => {
      ...
      const calc = computeSubject844(papers as any, numMarks, calcType, calcCfg);
  ...
  const setRemarks = (sid: string, v: string) =>
    setDraft((p) => ({ ...p, [sid]: { ...p[sid], papers: p[sid]?.papers || {}, remarks: v } }));

  const onSave = async () => {
    if (!papers.length) {
      toast.error("This subject has no papers configured. Add papers under Academics → Subjects.");
      return;
    }
    const items = rows
      .filter((r) => {
        const stored = parseStored(r.student.mark?.paper_scores);
        const dirty = papers.some((p) => {
    ...
    if (!items.length) { toast.info("Nothing to save"); return; }
    await save.mutateAsync({ assessment_id: task.assessment_id, task_id: task.id, items });
    setDraft({});
  };
  ...
  if (!papers.length) {
    return (
      <div className="p-6 text-center text-sm text-muted-foreground">
        This 8-4-4 subject has no papers configured. Open <b>Academics → Subjects</b>,
        find <b>{task.subject_name}</b>, and add its papers (e.g. Paper 1, Paper 2, Practical) first.
      </div>
    );
  }
    ...
    <div className="space-y-3">
      <div className="flex justify-end gap-2 flex-wrap">
        <Badge variant="secondary" className="gap-1">
          <Sparkles className="h-3 w-3" /> 8-4-4 · {calcType} · {papers.length} paper{papers.length > 1 ? "s" : ""}
        </Badge>
        <PermissionGate permission="exams:update">
          <Button variant="outline" onClick={onSave} disabled={locked || save.isPending}>
            ...
            <TableRow>
              ...
              {papers.map((p) => (
                <TableHead key={p.id} className="w-28 text-center">
                  {p.name}<div className="text-[10px] text-muted-foreground">/ {Number(p.max_marks)} {p.paper_type !== "THEORY" ? `· ${p.paper_type}` : ""}</div>
                </TableHead>
              ))}
          ...
          <TableBody>
            {rows.map((r) => (
              <TableRow key={r.student.id}>
                ...
                {papers.map((p) => (
                  <TableCell key={p.id}>
                    <Input
                      ...
                      disabled={locked}
                      onChange={(e) => setPaper(r.student.id, p.id, e.target.value)}
                ...
                <TableCell className="text-center tabular-nums">{r.calc.percentage}</TableCell>
                <TableCell className="text-center">
                  {r.grade ? <Badge>{r.grade.code}</Badge> : <span className="text-muted-foreground">—</span>}
                </TableCell>
                <TableCell className="text-center tabular-nums">{r.grade?.points ?? "—"}</TableCell>
                <TableCell>
                  <Input
                    value={r.remarks}
            ...
            {!rows.length && (
              <TableRow>
                <TableCell colSpan={papers.length + 7} className="text-center text-muted-foreground py-8">
