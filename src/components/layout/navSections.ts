...
export const NAV_SECTION_BY_URL: Record<string, string> = {
  ...
  "/finance": "Overview",
  "/fee-assignment": "Fees",
  "/finance/brought-forward": "Fees",
  "/finance/fee-report": "Fees",
  "/fee-discounts": "Fees",
  "/fee-adjustments": "Fees",
  "/fee-reminders": "Fees",
  "/payments": "Payments",
  "/excess-payments": "Payments",
  "/unallocated-payments": "Payments",
  "/payments/in-kind": "Payments",
  "/payments/bulk-bursary": "Payments",
  "/expenses": "Accounting",
  "/income": "Accounting",
...
export function sectionFor(url: string): string {
  return NAV_SECTION_BY_URL[url] || "More";
}

export function groupIntoSections<T extends { url: string }>(items: T[]) {
  const map = new Map<string, T[]>();
  for (const item of items) {
    const key = sectionFor(item.url);
